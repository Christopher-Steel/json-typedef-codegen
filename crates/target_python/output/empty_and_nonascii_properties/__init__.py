...
class Root:
    ...
    def from_json_data(cls, data: Any) -> 'Root':
        return cls(
            _from_json_data(str, data.get("")),
            _from_json_data(str, data.get("$foo")),
            _from_json_data(str, data.get("0foo")),
            _from_json_data(str, data.get("_foo")),
            _from_json_data(str, data.get("foo\nbar")),
            _from_json_data(str, data.get("foo bar")),
            _from_json_data(str, data.get("foo0bar")),
            _from_json_data(str, data.get("foo﷽bar")),
        )

    def to_json_data(self) -> Any:
        data: Dict[str, Any] = {}
        data[""] = _to_json_data(self.default_name)
        data["$foo"] = _to_json_data(self.foo)
        data["0foo"] = _to_json_data(self.foo0)
        data["_foo"] = _to_json_data(self.foo1)
        data["foo\nbar"] = _to_json_data(self.foo_bar)
        data["foo bar"] = _to_json_data(self.foo_bar0)
        data["foo0bar"] = _to_json_data(self.foo0bar)
        data["foo﷽bar"] = _to_json_data(self.foo_bar1)
        return data
